...
class Node:
    def __init__(self, up, down, left, right, column=None):
        self.up = up
        self.down = down
        self.left = left
        self.right = right
...
class ColumnHead(Node):
    def __init__(self, left, right, name: int, exact: bool = True):
...
class Problem:
    ...
    def __init__(self, tiles: list[tuple[int, int, int, int]], pieces: list[Piece]):
        # self._tiles = tiles
    ...
    def load_problem(self):
        ...
        def add_column_head(exact: bool, name):
            nonlocal left_node
            node = ColumnHead(left=left_node, right=None, exact=exact, name=name)
            left_node.right = node
            left_node = node
            return node

        piece_map = {}
        # Every piece is used exactly once if the number of pieces equals the number of tiles
        # Otherwise, every piece is used at most once
        exact_pieces = len(self._pieces) == self._num_tiles
        for i, piece in enumerate(self._pieces):
            piece_map[i] = add_column_head(exact=exact_pieces, name=piece)
        cubit_map = {}
        for i in sorted(set(self._cubits)):
            cubit_map[i] = add_column_head(exact=True, name=i)
        left_node.right = head
        head.left = left_node
        # Add rows
        rows = [(tile, (pc_idx, piece), orientation)
                for tile in range(self._num_tiles)
                for pc_idx, piece in enumerate(self._pieces)
                for orientation in Orientations]
        shuffle(rows)
        for (tile, (pc_idx, piece), orientation) in rows:
            piece.set_orientation(orientation)

            def add_node(column_head):
                nonlocal left_node
                up_node = column_head.up
                node = Node(up=up_node, down=column_head, left=left_node, right=None, column=column_head)
                up_node.down = node
                if left_node:
                    left_node.right = node
                column_head.up = node
                left_node = node
                if column_head.exact:
                    column_head.size += 1
                return node

            left_node = None
            first_node = add_node(piece_map[pc_idx])
            edge = piece.edge
            cubit_list = sorted(self._cubits[tile * 16 + i] for i, v in enumerate(edge) if v == 1)
            for cubit in cubit_list:
                add_node(cubit_map[cubit])
    ...
    def _add_hint(self, tile: int, piece: Piece):
        self._hints.append((tile, piece))
        cubit_set = {self._cubits[tile * 16 + i] for i, v in enumerate(piece.edge) if v == 1}
        c = next(
            c for c in Node.row_iterator(self._head) if c.name.color == piece.color and c.name.index == piece.index)
        Problem.cover(c)
        for c in Node.row_iterator(self._head):
            if c.exact and c.name in cubit_set:
                Problem.cover(c)

    def add_hints(self, hints):
        for tile, color, index, orientation in hints:
            piece = Piece(Pad[color][index], color=color, index=index)
            piece.set_orientation(orientation)
            self._add_hint(tile, piece)

    def _get_tile_assignment(self, row: Node):
        columns = [r.column.name for r in Node.row_iterator(row, include_first=True)]
        piece = next(element for element in columns if isinstance(element, Piece))
        cubit_columns = {c for c in columns if isinstance(c, int)}
        for tile in range(self._num_tiles):
            for orientation in Orientations:
                piece.set_orientation(orientation)
                cubit_set = {self._cubits[tile * 16 + j] for j, v in enumerate(piece.edge) if v == 1}
                if cubit_set == cubit_columns:
                    return tile, piece
    ...
    def solve(self):
        ...
        def search():
            try:
                selected_column = min((c for c in Node.row_iterator(self._head) if c.exact), key=lambda c: c.size)
            except ValueError:
                # No more columns to cover; problem solved
                yield sorted(chain((self._get_tile_assignment(row) for row in solution), self._hints))
            else:
                if selected_column.size == 0:
                    # No rows left to cover selected_column; solution not found
                    return
                Problem.cover(selected_column)
                for r in Node.column_iterator(selected_column):
                    solution.append(r)
                    for j in Node.row_iterator(r):
                        Problem.cover(j.column)
                    yield from search()
                    r = solution.pop()
                    for j in Node.row_iterator(r, reverse=True):
...
def solve(
    ...
    problem = Problem(tiles, pieces)
    if hints:
        problem.add_hints(hints)
...
def main():
    ...
    start = time.time()
    a = next(solve(PROBLEM6, [(pad.name, i) for pad in Pad for i in range(6)], sample(hints, 12)))
    print(', '.join([f'({tile}, "{p.color}", {p.index}, "{p.orientation}")' for tile, p in a]))
    for tile, p in a:
